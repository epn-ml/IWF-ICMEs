...
def get_lags(splitn):
    ...
    for i in range(1,splitn+1):
        with open('Results/startlag'+str(i)+'.p', 'rb') as fp:
            startlist = pickle.load(fp)
            starts = starts + startlist
    
        with open('Results/endlag'+str(i)+'.p', 'rb') as fp:
...
def get_cm(splitn):
    ...
    cm = np.loadtxt('Results/cm1.txt')
    
    for i in range(2,splitn+1):
        cm = cm + np.loadtxt('Results/cm'+str(i)+'.txt')
...
def get_eventcm(splitn):
    
    cmevent = np.loadtxt('Results/cmevent1.txt')
    
    for i in range(2,splitn+1):
        cmevent = cmevent + np.loadtxt('Results/cmevent'+str(i)+'.txt')
